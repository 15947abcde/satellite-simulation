...
# -*- coding: utf-8 -*-
"""
ALNS+TS scheduler (NO split + NO multi-satellite forward) - 核心算法模块
- 核心约束：
  1. 业务不可分片（单个任务必须在单一链路、单一连续时间段完成）
  2. 不允许多星转发（任务仅使用归属链路）
  3. 链路类型：1=RF，2-5=FSO(BPSK)
- 新增：GreedyDensity贪心算法作为ALNS初始解启发式
- 无输入输出逻辑，仅包含算法核心实现
"""
...
# ================= 配置类（统一管理参数）=================
class SchedulerConfigNoForward:
    def __init__(self):
        # 核心参数
        self.ITERATIONS = 500            # ALNS迭代次数
        self.SEED = 42
        ...
        # 速率映射（恢复原逻辑）
        self.RATE_MAP_GBPS = {1: 1.0, 2: 20.0, 3: 10.0, 4: 10.0, 5: 10.0}
        self.RATE_MAP = {k: v * 1e9 for k, v in self.RATE_MAP_GBPS.items()}
        self.LINK_TYPE = {1: "RF", 2: "FSO(BPSK)", 3: "FSO(BPSK)", 4: "FSO(BPSK)", 5: "FSO(BPSK)"}
        # 切换惩罚（无分片=无切换）
        self.MOD_SWITCH_TIME = 0.0
        self.LINK_SWITCH_TIME = 0.0
        # 贪心算法参数
        self.GREEDY_SORT_KEY = "value_density"  # value_density / value / size (可选)
        self.GREEDY_CANDIDATES = 5              # 贪心候选数
...
class LinkFTW:
    ...
    def add_segment(self, start: int, end: int, mod: int, config: SchedulerConfigNoForward):
        if end > start:
...
def load_tasks_from_dir(tasks_dir: str, config: SchedulerConfigNoForward) -> List[Task]:
    """加载任务数据（恢复原价值量逻辑）"""
    tasks = []
    files = []
    for ext in ("*.csv", "*.CSV"):
...
def load_link_ftw_csv(path: str, link_name: str, config: SchedulerConfigNoForward) -> LinkFTW:
    """加载链路可见性数据"""
    try:
        df = pd.read_csv(path)
...
def load_all_links(visibility_dir: str, config: SchedulerConfigNoForward) -> Dict[str, LinkFTW]:
    """加载所有链路数据"""
    link_map = {}
    csvs = []
    for ext in ("*.csv", "*.CSV"):
...
# ================= 核心调度器类 =================
class SchedulerNoForward:
    def __init__(self, tasks: List[Task], links: Dict[str, LinkFTW], device, config: SchedulerConfigNoForward, seed=42):
        self.tasks = tasks
        self.links = links
        self.device = device
        self.config = config
    ...
    def candidate_heuristic_cost(self, t: Task, chunk: Chunk) -> float:
        ...
        early_dur = max(0.0, min(chunk.finish, early_end) - max(chunk.start, t.arrival))
        early_bits = (early_dur / max(1e-9, chunk.finish - chunk.start)) * chunk.bits
    ...
    def try_pack_task_once(self, t: Task) -> Optional[Chunk]:
        ...
        if home_link not in self.links:
            return None

        need_bits = t.size_bits
        free_gaps = self.get_free_gaps(home_link, t.arrival, t.deadline)
        if not free_gaps:
            return None
        ...
        for seg in self.links[home_link].iter_segments_in_window(t.arrival, t.deadline):
            for gap_start, gap_end in self.get_free_gaps(home_link, seg.start, seg.end):
                max_bits_in_gap = seg.rate * (gap_end - gap_start)
                if max_bits_in_gap < need_bits:
                    continue

                task_duration = need_bits / seg.rate
                start = gap_start
                ...
                chunk = Chunk(
                    ...
                    mod_level=seg.mod_level,
                    link_type=seg.link_type
                )
        ...
        self.reserve(home_link, int(best_chunk.start), int(math.ceil(best_chunk.finish)), t.task_id)
        return best_chunk
    ...
    def _remove_assignments(self, removed: List[Assignment]) -> List[Assignment]:
        high_value_task_ids = {
            t.task_id for t in sorted(
                self.tasks,
                key=lambda x: x.value / max(1e-9, x.size_bits),
                reverse=True
            )[:int(len(self.tasks) * 0.2)]
        }

        real_removed = []

        for a in removed:
            if a.task_id in high_value_task_ids:
                continue

            real_removed.append(a)
            self.unscheduled.add(a.task_id)

            link = a.chunk.link
            self.slots_by_link[link] = [
                (s, e, tid)
                for (s, e, tid) in self.slots_by_link[link]
                if tid != a.task_id
            ]

        removed_ids = {a.task_id for a in real_removed}
        self.assignments = [
            x for x in self.assignments
            if x.task_id not in removed_ids
        ]
    ...
    def remove_cluster(self, k: int) -> List[Assignment]:
        ...
        for _, _, a in spans:
            ...
            if len(removed) >= min(k, len(self.assignments)):
                break
        removed = self._remove_assignments(removed)
        return removed
    ...
    def remove_worst_rf(self, k: int) -> List[Assignment]:
        ...
        real_removed = self._remove_assignments(removed)
        return real_removed
    ...
    def repair_random(self, max_trials: int = 256) -> int:
        ...
        cnt = 0
        for t in pending:
            # 关键：pending 生成后 unscheduled 会变化，这里要再确认一次
            if t.task_id not in self.unscheduled:
                continue
            ...
            if a:
                self.assignments.append(a)
                self.unscheduled.discard(t.task_id)  # discard 不会抛 KeyError
                cnt += 1
                if cnt >= max_trials:
                    break
        return cnt
    ...
    def run_alns_ts(self, print_every: int = 20) -> Tuple[float, List[Assignment], List[float]]:
        ...
        for a in self.assignments:
            self.reserve(a.chunk.link, int(a.chunk.start), int(math.ceil(a.chunk.finish)), a.task_id)
